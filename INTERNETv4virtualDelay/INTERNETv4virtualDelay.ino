...
bool b;
unsigned short timeNumber = 0;   // contador de tempo para virtual Delay
...
/* The 32 bit millis() and micros() timers counts from 0, 1, 2 to 
4294967295 (2^32-1) and than rollover to 0 again. The millis() timer is 
going to roll over after roughly 49.7 days, the micros() rolls over 
every 71.6 minutes. We have to take care about this rollover, because it
may happen that we start the virtual-delay just before the rollover time.
After the rollover the timestamps will be messed up. */
/* - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - -*/
// Global variables
...
/* - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - -*/ 
//  digitalWrite(LED_PIN, LOW);
  singleDelay.start(25000); 
// calls while running are ignored [25000 * 12] = 300000 = 5 min
//  if(singleDelay.elapsed()) {digitalWrite(LED_PIN, HIGH); 
  if(singleDelay.elapsed()) {digitalWrite(LED_PIN, b=!b); // blink the onboard LED 400ms, 400ms off
      Serial.println("... 1 vDelay COMUTANDO ESTADO DO LED ... "); // FIM SINGLEdELAY }
/* - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - -*/ 
